//using reducer and create context for exporting action in context Reducer
...
//Initializing initial state and loading values when the user logged in
...
export const Provider = ({ children }) => {
  ...
  //Using delete function for deleting our transaction
  const deleteTransaction = (id) => {
    dispatch({ type: 'DELETE_TRANSACTION', payload: id });
  };
//Using add function for adding our transaction
  const addTransaction = (transaction) => {
    dispatch({ type: 'ADD_TRANSACTION', payload: transaction });
  };
//Calculating balance amount as per the transaction done
